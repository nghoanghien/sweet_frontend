import { callCreateNewNhanVien } from "@/config/api";
import { IEmployeeFrontend, INhanVienReqDTO } from "@/types/employee";

export const createNewEmployee = async (data: IEmployeeFrontend): Promise<{
  success: boolean;
  message: string;
}> => {
  try {
    const newNhanVien: INhanVienReqDTO = {
      hoTen: data.fullName,
    ...
    const res = await callCreateNewNhanVien(newNhanVien);
