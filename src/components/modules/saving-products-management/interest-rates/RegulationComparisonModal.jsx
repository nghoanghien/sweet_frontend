...
const RegulationComparisonModal = ({
  ...
  const [activeSavingsType, setActiveSavingsType] = useState(
    newRegulation &&
      ...
      newRegulation.savingsTypes.length > 0
  ...
  useEffect(() => {
    ...
    const currentTypeIds = currentRegulation.savingsTypes.map(
      (type) => type.id
    );
  ...
  const activeNewSavingsTypeDetails = newRegulation?.savingsTypes?.find(
    (type) => type.id === activeSavingsType
  );

  // Find corresponding savings type in current regulation
  const activeCurrentSavingsTypeDetails = currentRegulation?.savingsTypes?.find(
    (type) => type.id === activeSavingsType
  );
  ...
  const confirmDetails = {
    ...
    "Số tiền gửi tối thiểu": newRegulation
      ? formatCurrency(newRegulation.minimumDeposit)
      : "",
    "Lãi suất không kỳ hạn": newRegulation
      ? `${newRegulation.noTermRate}%`
      : "",
  };
  ...
  useEffect(() => {
    if (
      ...
      !newRegulation ||
      !currentRegulation ||
      !activeCurrentSavingsTypeDetails ||
      !activeNewSavingsTypeDetails
    ) {
    ...
    activeCurrentSavingsTypeDetails.terms.forEach((term) => {
      ...
      currentRegulation.paymentFrequencies.forEach((freq) => {
        ...
        const isFreqDisabled =
          newRegulation.disabledFrequencies &&
            ...
            >
              ...
              <div className="bg-gradient-to-r from-blue-500 to-indigo-600 px-6 py-5 flex items-center justify-between rounded-t-3xl shadow-[0_4px_24px_rgba(0,170,255,0.10)]">
                ...
                >
                  ...
                  <span className="truncate">
                    Xác nhận thay đổi quy định lãi suất
                  </span>
                </motion.h2>
                <button
                  onClick={(e) => {
              ...
              <div className="flex-1 overflow-y-auto p-2 sm:p-6 max-h-[calc(96vh-110px)]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                  {/* Application date */}
                  <div
                    className={`bg-gradient-to-br from-blue-50/80 to-white p-5 rounded-2xl border shadow-[0_4px_24px_rgba(0,170,255,0.08)] ${
                      changedFields.applicationDate ||
                  ...
                  >
                    ...
                    <div className="flex items-center mb-2">
                      ...
                      {changedFields.minimumDeposit && (
                        <span className="ml-2 text-xs bg-green-100 text-green-800 px-2 py-0.5 rounded-full font-bold">
                          Thay đổi
                        </span>
                      )}{" "}
                    </div>{" "}
                    {changedFields.minimumDeposit ? (
                      <div className="flex items-center space-x-2">
                        {" "}
                        <p className="text-base font-semibold line-through text-red-600">
                          {formatCurrency(currentRegulation.minimumDeposit)}
                        </p>{" "}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  ...
                  <div className="bg-blue-50/60 p-4 rounded-2xl border border-blue-100 shadow-sm">
                    ...
                    <div className="text-xs text-gray-500 mb-1 font-semibold">
                      Mô tả quy định hiện tại
                    </div>{" "}
                    <div className="text-gray-700 italic min-h-[32px]">
                ...
                {/* Creator info */}
                <div className="bg-indigo-50 p-4 rounded-2xl mb-6 flex items-center border border-indigo-100 shadow-sm">
                  {" "}
                  <User
                    className="text-indigo-600 mr-2 flex-shrink-0"
                    size={20}
                  ...
                  <p className="text-indigo-800 font-semibold">
                    Người tạo quy định: {creator.name}
                  </p>{" "}
                </div>
                {/* Savings Types Changes Summary */}
                {(addedSavingsTypes.length > 0 ||
                  removedSavingsTypes.length > 0) && (
                  <div className="mb-6">
                    {" "}
                    <h3 className="text-lg font-bold text-gray-800 mb-3">
                      Thay đổi loại tiết kiệm
                    </h3>{" "}
                      ...
                      {addedSavingsTypes.length > 0 && (
                        <div className="bg-green-50 border border-green-200 rounded-2xl p-4 shadow-sm">
                          ...
                          <h4 className="text-sm font-bold text-green-700 mb-2 flex items-center">
                            {" "}
                            <Plus size={16} className="mr-2" /> Thêm loại tiết
                            kiệm mới{" "}
                          </h4>{" "}
                          <ul className="space-y-2">
                            {" "}
                            {addedSavingsTypes.map((type) => (
                              <li
                                key={type.id}
                      ...
                      {removedSavingsTypes.length > 0 && (
                        <div className="bg-red-50 border border-red-200 rounded-2xl p-4 shadow-sm">
                          ...
                          <h4 className="text-sm font-bold text-red-700 mb-2 flex items-center">
                            {" "}
                            <X size={16} className="mr-2" /> Xóa loại tiết kiệm{" "}
                          </h4>{" "}
                          <ul className="space-y-2">
                            {" "}
                            {removedSavingsTypes.map((type) => (
                              <li
                                key={type.id}
                                className="text-sm text-red-600 flex items-center font-semibold"
                              >
                                {" "}
                                <X size={14} className="mr-1.5" /> {type.name}{" "}
                              </li>
                            ))}{" "}
                          </ul>{" "}
                        </div>
                ...
                {newRegulation.savingsTypes.length > 0 && (
                  <SavingsTypeToggle
                    savingsTypes={newRegulation.savingsTypes}
                ...
                {activeNewSavingsTypeDetails &&
                  activeNewSavingsTypeDetails.terms.length > 0 && (
                    <div className="mt-4">
                      {" "}
                      <div className="flex items-center justify-between mb-4">
                        {" "}
                        <h3 className="text-lg font-bold text-gray-800">
                          So sánh quy định lãi suất
                        </h3>{" "}
                        <div className="hidden sm:flex items-center text-sm text-gray-600">
                          {" "}
                          <ArrowLeftRight size={16} className="mr-1" />{" "}
                          <span>Chuyển đổi loại tiết kiệm</span>{" "}
                        </div>{" "}
                      </div>{" "}
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                        {" "}
                        {/* Current regulation table */}{" "}
                        <div className="bg-white p-2 sm:p-4 rounded-2xl border border-red-100 shadow-[0_4px_24px_rgba(239,68,68,0.08)] overflow-x-auto">
                          {" "}
                          <div className="mb-3 px-4 py-2 bg-red-50 rounded-lg border border-red-100">
                            {" "}
                            <h3 className="text-sm font-bold text-red-700">
                              Quy định hiện tại (thay đổi sẽ hiển thị màu đỏ)
                            </h3>{" "}
                            <p className="text-xs text-red-600 mt-1">
                              Những dữ liệu được tô màu đỏ sẽ bị thay đổi hoặc
                              xóa trong quy định mới
                            </p>{" "}
                          </div>{" "}
                          {activeCurrentSavingsTypeDetails ? (
                            <InterestRateTable
                              savingsType={activeCurrentSavingsTypeDetails.name}
                              savingsTypeId={activeCurrentSavingsTypeDetails.id}
                              terms={activeCurrentSavingsTypeDetails.terms}
                              interestRates={
                                activeCurrentSavingsTypeDetails.interestRates
                              }
                              paymentFrequencies={
                              ...
                              onAddTerm={() => {}}
                              onRemoveTerm={() => {}}
                              highlightChanges={oldTableHighlights}
                              highlightColor="red"
                            />
                          ) : (
                            <div className="text-center py-8 text-gray-500">
                              {" "}
                              {removedSavingsTypes.some(
                                (type) => type.id === activeSavingsType
                              ) ? (
                        ...
                        {/* New regulation table */}{" "}
                        <div className="bg-white p-2 sm:p-4 rounded-2xl border border-green-200 shadow-[0_4px_24px_rgba(34,197,94,0.08)] overflow-x-auto">
                          {" "}
                          <div className="mb-3 px-4 py-2 bg-green-50 rounded-lg border border-green-100">
                            {" "}
                            <h3 className="text-sm font-bold text-green-700">
                              Quy định mới (thay đổi sẽ hiển thị màu xanh)
                            </h3>{" "}
                            <p className="text-xs text-green-600 mt-1">
                              Những dữ liệu được tô màu xanh là dữ liệu mới được
                              thêm vào hoặc thay đổi so với quy định cũ
                            </p>{" "}
                          </div>{" "}
                          {activeNewSavingsTypeDetails ? (
                            <InterestRateTable
                              savingsType={activeNewSavingsTypeDetails.name}
                              savingsTypeId={activeNewSavingsTypeDetails.id}
                              terms={activeNewSavingsTypeDetails.terms}
                              interestRates={
                                activeNewSavingsTypeDetails.interestRates
                              }
                              paymentFrequencies={
                                newRegulation.paymentFrequencies
                              }
                              disabledFrequencies={
                                newRegulation.disabledFrequencies || []
                              }
                              ...
                              onAddTerm={() => {}}
                              onRemoveTerm={() => {}}
                              highlightChanges={newTableHighlights}
                              highlightColor="green"
                            />
                          ) : (
                            <div className="text-center py-8 text-gray-500">
                              {" "}
                              {addedSavingsTypes.some(
                                (type) => type.id === activeSavingsType
                              ) ? (
                                <div className="flex flex-col items-center">
                                  {" "}
                                  ...
                                  <p className="text-green-600 font-bold">
                                    Loại tiết kiệm mới được thêm vào
                                  </p>{" "}
                                </div>
                              ) : (
                                <div>Không có dữ liệu</div>
