...
import PaymentAccountsNew from './PaymentAccountsNew';
import SavingsAccounts from './SavingsAccounts';
import CalendarDatePicker from './CalendarDatePicker';
import ExportDataModal from './ui/export-modal/ExportDataModal';
import ExportNotification from './ui/notifications/ExportNotification';
import InputField from './ui/custom/Inputfield';
import CustomSelect from './ui/custom/CustomSelect';
import AddressFields from './ui/custom/AddressFields';
import StatusBadge from './ui/custom/StatusBadge';
import ModalHeader from './ui/custom/ModalHeader';
...
export default function CustomerManagement() {
  ...
  // Get savings transaction type information
  const getSavingsTransactionTypeInfo = (transaction) => {
    if (transaction.isDeposit) {
      ...
      return {
        icon: <MinusCircle size={20} className="text-amber-600" />,
        bgColor: "bg-amber-100",
        textColor: "text-amber-600"
      };
    } else {
  ...
  // Function to close transaction drawer
  ...
  // Function to get channel info for transaction drawer
  const getChannelInfo = (channel) => {
    switch(channel) {
      case 'Internet Banking': return { icon: <Globe size={14} className="text-blue-500 mr-1" />, name: channel };
      case 'Mobile Banking': return { icon: <Smartphone size={14} className="text-green-500 mr-1" />, name: channel };
      case 'ATM': return { icon: <CreditCard size={14} className="text-purple-500 mr-1" />, name: channel };
      case 'Quầy giao dịch': return { icon: <Building size={14} className="text-amber-500 mr-1" />, name: channel };
      case 'Chuyển khoản liên ngân hàng': return { icon: <FileText size={14} className="text-indigo-500 mr-1" />, name: channel };
      default: return { icon: <FileText size={14} className="text-gray-500 mr-1" />, name: channel };
    }
  };
